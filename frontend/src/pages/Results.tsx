...
import { FileSpreadsheet, RefreshCw, Brain, Download } from "lucide-react";
...
export default function ResultsPage() {
  ...
  const openAssistant = (jobId: string) => {
    navigate(`/assistant?job_id=${jobId}`);
  };
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-red-500/20 border border-red-500/50 text-red-200 p-4 mb-4 rounded-md">
          ...
          <p className="text-sm mt-1 text-gray-300">Using fallback data for demonstration</p>
        </div>
      ) : (
        <>
          {resultsData.length === 0 ? (
            <div className="text-center py-12 bg-background-light/10 rounded-lg border border-background-light">
              <FileSpreadsheet size={48} className="mx-auto text-gray-500 mb-3" />
              <p className="text-gray-400 mb-2">No results found</p>
              <p className="text-sm text-gray-500">Upload survey documents to see results here</p>
            </div>
          ) : (
            <div className="bg-background-light/5 border border-background-light rounded-lg overflow-hidden">
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-background-light/20">
                  {resultsData.map((row: any, idx: number) => (
                    <tr key={idx} className="hover:bg-background-light/10 transition-colors">
                      <td className="px-6 py-4 font-medium">{row.sheet_name}</td>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-wrap gap-2">
                          ...
                          {row.anomalies > 0 && (
                            <button
                              onClick={() => openAssistant(row.job_id)}
      ...
      {selectedJob && <XLSXPreview jobId={selectedJob} onClose={closePreview} />}
    </div>
  );
} 