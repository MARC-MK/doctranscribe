...
const AssistantDrawer: React.FC<AssistantDrawerProps> = ({ jobId, onClose }) => {
  ...
  useEffect(() => {
    if (jobContext && jobContext.anomalies.length > 0) {
      ...
      const sortedAnomalies = [...jobContext.anomalies].sort((a, b) => b.score - a.score);
      ...
      if (anomalyCount > 3) {
        recommendedAction = "I recommend starting with a summary view since there are multiple anomalies.";
        ...
        recommendedAction = "I recommend a row-by-row review to examine each anomaly in detail.";
      }
  ...
  const sendMessage = async (message: string) => {
    ...
    try {
      ...
      if (response.data?.assistant_messages) {
        const newMessages = response.data.assistant_messages.map((msg: any) => ({
          role: msg.role,
          content: msg.content
        }));
    ...
    return (
      <div className="flex flex-wrap gap-2 mt-4">
        ...
        {jobContext.anomalies.length > 0 && (
          <button
            onClick={() => handleQuickAction(`Tell me about ${jobContext.anomalies[0].row_id}`)}
              ...
              <div className="flex h-full flex-col bg-white shadow-xl">
                ...
                <div className="border-b border-gray-200 px-4 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-semibold text-gray-800">Anomaly Assistant</h2>
                  </div>
                  <button 
                    onClick={handleClose}
                  ...
                  {isLoading && messages.length === 0 ? (
                    <div className="flex items-center justify-center h-32">
                      <div className="flex flex-col items-center">
                        ...
                        <p className="text-gray-500 text-sm">Analyzing anomalies...</p>
                      </div>
                    </div>
                  ) : (
                    <>
                      ...
                      {messages.length === 1 && !isLoading && getQuickActionButtons()}
                    </>
                  )}
                </div>
